...
def interview():
    ...
    return render_template('interview.html',name=name,display_button=False)
...
def audio_recording_interview():
    ...
    return render_template('interview.html', display_button=True,name=name,text=text)
...
def interview_analysis():
    ...
    score = 0.6*(emotion_dist[0]+emotion_dist[1]+emotion_dist[2]+emotion_dist[3]+emotion_dist[4]+0.2*emotion_dist[5]) + 0.4*(probas[0]+probas[1]+probas[2]+probas[3]+probas[4]) 

    

    return render_template('score_analysis.html', a_emo=major_emotion, a_emo_other=major_emotion_other, a_prob=emotion_dist, a_prob_other=emotion_dist_other,t_text=text, t_traits = probas, t_trait = trait, t_trait_others = trait_others, t_probas_others = probas_others, t_num_words = num_words, t_common_words = common_words_perso, t_common_words_others=common_words_others,name=name,position=position, score=score)
